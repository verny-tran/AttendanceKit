...
- (void) loadModel:(NSString*)graphFileName
            labels:(NSString*)labelsFileName
         memMapped:(bool)map optEnv:(bool)opt {
    [self createQueue];
    ...
    dispatch_sync(tfQueue, ^{
        ...
        for (int y = 0; y < wanted_input_height; ++y) {
            ...
            for (int x = 0; x < wanted_input_width; ++x) {
                tensorflow::uint8 *in_pixel = inn + (y * image_width * image_channels) + (x * image_channels);
        ...
        if (self -> tf_session.get()) {
            ...
            tensorflow::Status run_status = self -> tf_session -> Run({{
                self -> input_layer_name, image_tensor
            }}, { self -> output_layer_name }, {}, &outputs);
...
-(void) createQueue {
    if (tfQueue == nil) {
        tfQueue = dispatch_queue_create("tfQueue",
