...
| For connecting to RFID antenna to read and display tag IDs, act as an admin app to compose notifications for mobile devices. | Face recognition is used as biometric security for all students when checking attendance. | To collect student face samples, class scheduling, assign tags, and classrooms. | For lecturers to view attendance reports and change schedules. |
